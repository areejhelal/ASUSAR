...
static void InitialiseNewTask ( UBaseType_t TaskID )
{
    ...
    if ( NewTCB->TaskType == extended )
    {
        ...
        /* TODO: re-factoring the implementation [pointer to pointer]*/
    ...
    uxSavedInterruptStatus = portSET_INTERRUPT_MASK_FROM_ISR();
    {
        ...
        if( ChainTask != pdFALSE )
        {
            ...
            if (pxCurrentTCB->TaskRequest > 1)
            {
                ...
                /*MISRA Rules:
                 * if function doesn't return, cast as void */
                ( void ) uxListRemove( &( pxCurrentTCB->StateListItem ) );
                vListInsertEnd( &( ReadyTasksLists[ pxCurrentTCB->Priority ] ), &( pxCurrentTCB->StateListItem ) );

                /*
                 * TODO:
                 * xYieldRequired should be set to pdTRUE independent of the condition of pxCurrentTCB->TaskRequest value
                 * xYieldRequired = pdTRUE;
                 *
                 * */
            }
        ...
        if ( (ChainTask == pdFALSE) || (pxCurrentTCB->TaskRequest == NULL) )
        {
            if( listIS_CONTAINED_WITHIN( &SuspendedTaskList, &( pxTCB->StateListItem ) ) != pdFALSE )
            {
                ...
                if( pxTCB->Priority >/*=*/ pxCurrentTCB->Priority )
                {
                    xYieldRequired = pdTRUE;
                    /*TODO:
                     * when ">=" the  xYieldRequired = pdTRUE, which cause a scheduler call
                     * scheduler fails to run the right task
                     * */
                }
                else
                {
                ...
                if ((pxTCB->TaskRequest) < (pxTCB->Activation))
                {
                    pxTCB->TaskRequest++; /*if task within suspended state, we should not increment the TaskRequest*/
                }
            }
            else
            {
...
StatusType ChainTask (TaskType TaskID)
{
    ...
    /* The context switch should be performed if
    the current task defined as full preemption */
    if ( (TaskResume( pxTCB, pdTRUE ) != pdFALSE) && (pxCurrentTCB->Schedule != NON_PREEMPTIONTASK) )
    {
        portYIELD();
    }
    else
